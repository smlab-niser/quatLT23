...
def one_epoch(
    ...
    batch_x, batch_y = batch_x.to(GPU), batch_y.float().to(GPU)
    optimiser.zero_grad()
...
def train_accuracy(
    ...
    for batch_x, batch_y in data_generator:
        batch_x, batch_y = batch_x.to(GPU), batch_y.argmax(1).numpy()
        output = model(batch_x).argmax(1).cpu().numpy()
        # print(f"{output.shape = }, {batch_y.shape = }")
        acc = accuracy_score(batch_y, output)
        accs.append(acc*100)
        j += 1
        if j == n: break
